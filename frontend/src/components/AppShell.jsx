...
export default function AppShell() {
  ...
  const navCls = ({ isActive }) =>
    cn("flex items-center gap-2.5 rounded-md px-3 py-2 text-sm transition-colors duration-150",
      isActive ? "bg-[#27272a] text-zinc-100" : "text-zinc-400 hover:text-zinc-100 hover:bg-[#1c1c1f]");

  const initials = (user?.name || user?.email || "?").slice(0, 2).toUpperCase();

  return (
    <div className="flex h-screen bg-[#09090b] text-zinc-100 overflow-hidden">
      <aside className="hidden md:flex w-64 flex-col border-r border-zinc-800/80 bg-[#111113]">
        ...
        <nav className="flex-1 overflow-y-auto thin-scroll p-3 space-y-1">
          ...
          {loading ? (
            <div className="px-3 py-2"><Loader2 className="h-4 w-4 animate-spin text-zinc-600" /></div>
          ) : projects.length === 0 ? (
            <p className="px-3 py-2 text-xs text-zinc-600">Chưa có dự án</p>
          ) : (
            projects.map((p) => (
        ...
        <div className="border-t border-zinc-800/80 p-3">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 hover:bg-[#1c1c1f] transition-colors" data-testid="user-menu-trigger">
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-600 text-xs font-semibold">{initials}</div>
                <div className="flex-1 text-left overflow-hidden">
                  <div className="truncate text-sm">{user?.name}</div>
                  <div className="truncate text-xs text-zinc-500">{ROLE_LABEL[user?.role] || user?.role}</div>
                </div>
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-52 bg-[#18181b] border-zinc-800">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        {/* mobile top bar */}
        <div className="md:hidden flex h-14 items-center justify-between border-b border-zinc-800/80 bg-[#111113] px-4">
          ...
          <button onClick={logout} data-testid="logout-btn-mobile"><LogOut className="h-5 w-5 text-zinc-400" /></button>
        </div>
