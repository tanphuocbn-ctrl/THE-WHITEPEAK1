...
export default function Budget() {
  ...
  if (!data) return <div className="flex h-64 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-zinc-600" /></div>;
  ...
  return (
    <div className="mx-auto max-w-4xl animate-fade-up space-y-6">
      <div className="rounded-lg border border-zinc-800/80 bg-[#18181b] p-5">
        <div className="flex items-center gap-2 mb-4"><Wallet className="h-5 w-5 text-zinc-500" /><h2 className="font-head font-bold">Ngân sách dự án</h2></div>
        {canWrite && (
          <div className="flex flex-wrap items-end gap-3 mb-4">
            <div className="flex-1 min-w-[200px] space-y-2"><Label>Tổng ngân sách (VND)</Label>
              <Input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} data-testid="budget-amount" className="bg-[#0f0f11] border-zinc-800 tabular" /></div>
          ...
          <div className="space-y-2">
            {data.by_sequence.map((s) => (
              <div key={s.sequence_id} className="flex items-center justify-between text-sm">
                <span className="text-zinc-300">{s.code !== "—" ? `${s.code} · ` : ""}{s.title}</span>
      ...
      <div className="rounded-lg border border-zinc-800/80 bg-[#18181b]">
        <div className="border-b border-zinc-800/80 p-4"><h3 className="font-head font-semibold">Chi phí</h3></div>
        {canWrite && (
          <div className="flex flex-wrap gap-2 p-4 border-b border-zinc-800/80">
            <Input value={exp.title} onChange={(e) => setExp({ ...exp, title: e.target.value })} placeholder="Hạng mục chi" data-testid="exp-title" className="flex-1 min-w-[160px] bg-[#0f0f11] border-zinc-800" />
            <Input type="number" value={exp.amount} onChange={(e) => setExp({ ...exp, amount: e.target.value })} placeholder="Số tiền" data-testid="exp-amount" className="w-32 bg-[#0f0f11] border-zinc-800 tabular" />
            <Select value={exp.category} onValueChange={(v) => setExp({ ...exp, category: v })}>
              <SelectTrigger data-testid="exp-cat" className="w-36 bg-[#0f0f11] border-zinc-800"><SelectValue /></SelectTrigger>
            ...
            <Select value={exp.sequence_id} onValueChange={(v) => setExp({ ...exp, sequence_id: v })}>
              <SelectTrigger data-testid="exp-seq" className="w-40 bg-[#0f0f11] border-zinc-800"><SelectValue placeholder="Sequence" /></SelectTrigger>
              <SelectContent className="bg-[#18181b] border-zinc-800">
        ...
        {data.expenses.length === 0 ? (
          <p className="p-6 text-center text-sm text-zinc-500">Chưa có chi phí nào.</p>
        ) : (
          <div className="divide-y divide-zinc-800/80">
            {data.expenses.map((e) => (
              <div key={e.id} className="flex items-center gap-3 p-4" data-testid={`exp-${e.id}`}>
                <span className="rounded-full border border-zinc-700 px-2 py-0.5 text-xs text-zinc-400">{e.category}</span>
                <span className="flex-1 truncate text-sm">{e.title}</span>
                <span className="tabular text-sm text-amber-300">{money(e.amount)}</span>
                <span className="hidden sm:block text-xs text-zinc-500">{fmtDate(e.created_at)}</span>
                {canWrite && <Button size="icon" variant="ghost" onClick={() => delExpense(e.id)} className="text-zinc-500 hover:text-red-400"><Trash2 className="h-4 w-4" /></Button>}
              </div>
            ))}
          </div>
        )}
