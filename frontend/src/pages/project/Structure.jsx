    ...
    <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6 animate-fade-up">
      ...
      <div className="rounded-lg border border-[var(--border)] bg-[var(--panel)]">
        ...
        <div className="p-2 max-h-[70vh] overflow-y-auto thin-scroll">
          {loading ? (
            <div className="p-4"><Loader2 className="h-4 w-4 animate-spin text-[var(--muted-2)]" /></div>
          ) : sequences.length === 0 ? (
            <p className="p-3 text-sm text-[var(--muted-2)]">Chưa có sequence. {canWrite && "Tạo sequence để bắt đầu."}</p>
          ) : sequences.map((sq) => {
            ...
            return (
              <div key={sq.id} className="mb-1">
                <button onClick={() => setExpanded({ ...expanded, [sq.id]: !isOpen })}
                  ...
                  className="flex w-full items-center gap-1.5 rounded-md px-2 py-1.5 text-sm hover:bg-[var(--panel-2)] transition-colors">
                  {isOpen ? <ChevronDown className="h-3.5 w-3.5 text-[var(--muted-2)]" /> : <ChevronRight className="h-3.5 w-3.5 text-[var(--muted-2)]" />}
                  <Film className="h-4 w-4 text-[var(--muted-2)]" />
                  ...
                  <div className="ml-4 border-l border-[var(--border)] pl-2">
                    {seqScenes.map((sc) => (
                      <button key={sc.id} onClick={() => selectScene(sc)} data-testid={`scene-${sc.code}`}
                        className={`flex w-full items-center gap-1.5 rounded-md px-2 py-1.5 text-sm transition-colors ${selScene?.id === sc.id ? "bg-[var(--panel-2)] text-[var(--text)]" : "text-[var(--muted)] hover:bg-[var(--panel-2)] hover:text-[var(--text)]"}`}>
      ...
      <div>
        {!selScene ? (
          <div className="flex h-64 flex-col items-center justify-center rounded-lg border border-dashed border-[var(--border)] text-center">
            ...
            <p className="text-[var(--muted)]">Chọn một scene để xem chi tiết và shots.</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="rounded-lg border border-[var(--border)] bg-[var(--panel)] p-5">
              <p className="font-mono text-xs text-[var(--muted-2)]">{selScene.code}</p>
              <h2 className="font-head text-xl font-bold mt-0.5">{selScene.title}</h2>
              <div className="mt-3 grid grid-cols-2 gap-4 text-sm">
                <div><span className="overline block mb-1">Bối cảnh</span><span className="text-[var(--muted)]">{selScene.location || "—"}</span></div>
                <div><span className="overline block mb-1">Thời điểm</span><span className="text-[var(--muted)]">{selScene.time_of_day || "—"}</span></div>
              </div>
              {selScene.description && <p className="mt-3 text-sm text-[var(--muted)] whitespace-pre-line">{selScene.description}</p>}
            </div>
            ...
            <div className="rounded-lg border border-[var(--border)] bg-[var(--panel)]">
              ...
              {shots.length === 0 ? (
                <p className="p-6 text-center text-sm text-[var(--muted-2)]">Chưa có shot trong scene này.</p>
              ) : (
                <div className="divide-y divide-[var(--border)]/80">
                  {shots.map((sh) => (
                    <button key={sh.id} onClick={() => setOpenShot(sh.id)} data-testid={`shot-row-${sh.code}`}
                      className="flex w-full items-center gap-3 p-4 text-left hover:bg-[var(--panel-2)] transition-colors">
                      <span className="font-mono text-sm text-blue-400 w-16">{sh.code}</span>
                      <span className="flex-1 truncate text-sm">{sh.title}</span>
                      <span className="hidden sm:block text-xs text-[var(--muted-2)]">{sh.assignee_name || "chưa giao"}</span>
                      <span className="hidden sm:block text-xs text-[var(--muted-2)] tabular">{fmtDate(sh.deadline)}</span>
          ...
          {fields.map((f) => (
            <div key={f.key} className="space-y-2">
              ...
              <Input value={vals[f.key] || ""} onChange={(e) => setVals({ ...vals, [f.key]: e.target.value })}
                ...
                className={`bg-[var(--panel-2)] border-[var(--border)] ${f.mono ? "font-mono" : ""}`} />
            </div>
          ))}
        </div>
        <DialogFooter>
...
const NewScene = ({ projectId, sequenceId, onDone }) => (
  <DialogForm testid="new-scene" title="Tạo Scene"
    trigger={<Button size="sm" variant="ghost" className="text-[var(--muted-2)] h-7 w-full justify-start px-2" data-testid="new-scene-btn"><Plus className="h-3.5 w-3.5 mr-1" /> Scene</Button>}
